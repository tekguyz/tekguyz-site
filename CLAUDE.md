...
**Never push to `master`. Open a PR.** `git push origin master` IS a production deploy — one Vercel project, `tekguyz-site`, holds both `tekguyz.com` and `www.tekguyz.com`, and every push to `master` auto-deploys to it. There is **no preview project** between the repo and live traffic; `tekguyz-website` does not exist, whatever older notes say. So the branch is the approval gate:
...
**Merging is the deploy**, and it is the user's to perform — not yours, and not something a "yes, commit and push" implies. Adopted 2026-08-10 after a push to `master` reached production while the session was still describing it as pending: the classifier denial that was reported as a block had landed *after* git completed. **A denied push is not a push that didn't happen — measure `git log origin/master` or `list_deployments`, don't infer from the error.**
