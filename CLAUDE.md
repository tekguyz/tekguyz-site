...
- **`bun run check:design` gates `prebuild`.** It asserts that every token DESIGN.md prints in a fenced `--token: value;` block actually matches `app/globals.css` — 24 of them across §2.1 type, §3.1 radius/container, §6.1 motion, §8.0 density. A mismatch fails the build and names the token and both values. **This is the mechanism that makes DESIGN.md quotable**: a value it prints in those blocks cannot silently drift. Adding a section to the guard is a two-line change in `scripts/check-design.ts`. It cannot check prose, and does not pretend to.
