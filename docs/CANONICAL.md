...
- **Confirmed CRM contract (from the CRM team, load-bearing):**
  ...
  - Responses: `200 {success, leadId}` · `400` invalid JSON · `400` invalid payload with field issues · **`404` for an unknown or bad secret, which is indistinguishable from a wrong URL** · `429` rate limit + `Retry-After: 60`. Allowed request header is `Content-Type` **only** — a custom header fails preflight.
