...
- `app/actions/contact.ts` — port as *the base structure*, but the field set changes — see the confirmed CRM contract immediately below. One shared lead-capture action, called by both the form and the concierge with a different `source`. Never a second implementation.
  - **The action returns as soon as validation and the rate limit pass — not after the CRM write or either email** (2026-08-11, in response to the endpoint's own 2–5s latency and a measured 7.36s round trip when everything was awaited). The CRM write and both Resend sends run inside `after()` from `next/server`, so nothing downstream of the response can change what the visitor is told. Consequence: **the result no longer depends on Resend at all.** It used to return `{ success: false }` on a notification failure even when the CRM write had already succeeded — which, given the upsert-by-email contract above, meant a visitor told to retry was overwriting their own already-captured lead with a second, usually shorter attempt. The CRM row is the lead; email is a notification about it.
  - **Every failure inside `after()` is recorded twice, because the visitor is already gone by the time it happens**: a greppable console marker (`[LEAD-DELIVERY-FAILURE]`, carrying the email and an ISO timestamp) and the full payload persisted to Upstash via `lib/lead-archive.ts` (90-day TTL, newest-first index). No queue, no retry — after() plus one Upstash write is the whole mechanism, and the CRM write specifically is never retried, per the upsert warning above.
  - **The honeypot's silent-accept branch is now observable without becoming visible to the bot that tripped it.** `[LEAD-HONEYPOT]` logs field lengths and fill time, never the honeypot's own value (attacker-controlled text).
  - **A form-filling bot reads placeholders and page copy off the DOM and submits them as content.** Two real leads did this: one carried the `/contact` hero subhead as its message, one carried 160 characters of scraped copy as its name — which is what tripped the CRM's spam shield on genuine enquiries too. `lib/validation.ts` strips known site copy from `message` before validation (`stripUiCopy`) and rejects it outright from `name` (`personName`'s shape check) — see that file for why both live there rather than in the action.
...
- **Confirmed CRM contract (from the CRM team, load-bearing):**
  - **SUPERSEDED 2026-08-18 — the endpoint is now `POST /api/v1/triage/[organization_id]` and every request must be signed.** The URL no longer carries the secret and grants nothing on its own; authentication is an `X-TekGuyz-Signature` header holding the hex HMAC-SHA256 of the **raw request body**, keyed by `CRM_SIGNING_SECRET`. `sendToCrm` serializes the payload once and signs that exact string — a re-serialized copy 401s every time. Rotating the secret in the CRM is immediate with no grace period, and the endpoint URL does not change when it rotates. **SUPERSEDED IN PART 2026-09-14 — the signature is now timestamped.** The HMAC covers `${timestamp}.${rawBody}`, and the same timestamp (Unix seconds, digits only) travels in `X-TekGuyz-Timestamp`. The CRM refuses a body-only signature, a timestamp more than 300 seconds either side of its clock, and a signature it has already seen. Hard cutover, no dual support: the two sides deploy together. Site side shipped in `3f8dda0`, confirmed receiving leads. Full protocol: `src/lib/webhooks/signature.ts` in the CRM repo — ~~`docs/WEBHOOK_INTEGRATION.md`~~, removed from the CRM on 2026-08-19.
  - **The CORS paragraph that used to sit here was wrong about this call, and it cost a real capability.** CORS and preflight are browser mechanisms; `sendToCrm` is a server-side fetch from a Server Action, so it has no origin and never triggers a preflight. The CRM's allowed-origin list has never applied to it. That misreading is why the code carried "a custom header fails preflight" as a comment and why a signed header looked impossible from this side. A domain or hosting change does **not** break this call.
  ...
  - **UPSERT BY EMAIL — PARTLY SUPERSEDED 2026-08-17, read the correction first.** The CRM stopped overwriting identity columns on a resubmission: `client_name`, `phone`, `company`, `website`, `physical_address`, `service_category` and `lead_source` are now **first-known values**, written once when the contact is created and never again by ingestion, and each enquiry gets its own immutable `lead_submissions` row. So a returning visitor's first enquiry is no longer destroyed. Still true and still load-bearing: **one lead row per email** (a resubmission returns the same `leadId`), and the reactivation-to-`NEW` behaviour. The site-side rules below are unchanged and stay — fire exactly once, never retry, never tell a visitor to try again after a write that may have landed — but the *reason* is now "no duplicate rows and no retry storms" rather than "a retry destroys data". The original 2026-08-11 text follows for the record. — A resubmission from an address already in the CRM does **not** create a second row. It overwrites `client_name`, `phone`, `company`, `website`, `physical_address`, `service_category` and `lead_source` on the existing row, and reactivates it to `NEW` if archived. A returning visitor's first enquiry is **silently destroyed, not duplicated** — which is worse than a duplicate, not better. Two consequences the site side must honour: the CRM write **fires exactly once per submission and is never retried** (a retry from stale data can overwrite a row a later submission already corrected), and the visitor is **never** told to "try again" after a write that may have landed. Confirmed live on 2026-08-11: five test submissions from one address returned the same `leadId` five times.
  - **Latency: 2–5s, by design, and it will not get faster from this side** (measured 3.8s, 5.1s, and again on 2026-08-11). The endpoint awaits its own Gemini spam-shield call (8s timeout) and its own Resend send before responding. Budget for it entirely inside `after()` — see the lead-capture rules in CLAUDE.md. Anything that awaits this endpoint before returning to the visitor has put a third party's LLM call on the critical path of the site's only conversion.
  - Responses (updated 2026-08-18): `200 {success, leadId}` · `400` invalid JSON · `400` invalid payload with field issues · **`401` for any authentication failure — unknown org id, missing header, malformed or wrong signature, and since 2026-09-14 a missing, stale or future timestamp and a replayed signature, all returning the same opaque body on purpose** · `429` rate limit + `Retry-After: 60`. The old `404` for a bad secret is gone with the old scheme. A `400` therefore means the signature was already accepted, which makes it the fastest way to prove the signing code works.
  - **The CRM enforces no length or shape on `client_name`.** 160 characters of scraped page prose is a valid name to it. The check has to live on this side; it does, in `lib/validation.ts`.
...
- **Rotate the Resend key** (exposed in plaintext). ✅ The CRM triage endpoint half of this is **done, 2026-08-18** — it now requires an HMAC-SHA256 request signature, which is stronger than the shared-secret header this line asked for, since the credential is never transmitted at all.
...
CRM_TRIAGE_ENDPOINT=       # https://tekguyz-crm.vercel.app/api/v1/triage/[organization_id] — not a secret
CRM_SIGNING_SECRET=        # THIS is the CRM credential (2026-08-18) — keys the request signature
