...
| **`bun run check:claude` fails on the test count, and the cause is a stray git worktree, not the doc.** Measured 2026-09-20: `CLAUDE.md` says 107 cases across 5 files; `bun run test` reports 214 across 10. `git ls-files "*.test.ts"` returns **5** files. The other 5 are duplicates under `.claude/worktrees/tekguyz-project-ideas-72744b/`, which `vitest` walks. So 107/5 is correct and the guard is measuring the worktree. Fix is to scope `vitest`'s include/exclude, or remove the worktree — **not** to edit the figure in `CLAUDE.md`. Deferred out of the Job 3 rename commit on 2026-09-20 because a doc-guard repair is `doc-audit`'s job and its commit rule is one doc alone | 1 |
